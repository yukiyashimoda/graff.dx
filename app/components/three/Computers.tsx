...
const context = createContext<Record<string, React.ComponentType<Record<string, unknown>>> | null>(null)

export function Instances({ children, ...props }: { children: React.ReactNode; [key: string]: unknown }) {
  const { nodes } = useGLTF('/computers_1-transformed.glb') as unknown as { nodes: Record<string, THREE.Mesh> }
  ...
  return (
    <Merged castShadow receiveShadow meshes={instances} {...props}>
      {(inst: Record<string, React.ComponentType<Record<string, unknown>>>) => (
        <context.Provider value={inst}>{children}</context.Provider>
      )}
    </Merged>
  )
}

export function Computers(props: Record<string, unknown>) {
  const { nodes: n, materials: m } = useGLTF('/computers_1-transformed.glb') as unknown as {
...
function Screen({
  ...
  const { nodes, materials } = useGLTF('/computers_1-transformed.glb') as unknown as {
    nodes: Record<string, THREE.Mesh>
    materials: Record<string, THREE.Material>
  }
  return (
...
function ScreenInteractive(props: { frame: string; panel: string; [key: string]: unknown }) {
  return (
    <Screen {...props}>
...
function Leds({ instances }: { instances: Record<string, React.ComponentType<Record<string, unknown>>> }) {
