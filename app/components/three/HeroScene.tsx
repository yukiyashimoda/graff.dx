...
export default function HeroScene({ active }: { active: boolean }) {
  return (
    ...
    >
      ...
      <spotLight position={[10, 20, 10]} angle={0.12} penumbra={1} intensity={3} castShadow shadow-mapSize={512} />
      ...
      <EffectComposer disableNormalPass {...({} as any)}>
        <Bloom luminanceThreshold={0.2} luminanceSmoothing={0.1} intensity={2.5} />
      </EffectComposer>
      <CameraRig />
      <BakeShadows />
    </Canvas>
...
function Bun(props: Record<string, unknown>) {
  ...
  return (
    <mesh castShadow geometry={nodes.mesh.geometry} {...props}>
